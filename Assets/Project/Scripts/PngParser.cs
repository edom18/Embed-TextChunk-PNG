...
[BurstCompile]
public struct PngParseJob : IJob
{
    [ReadOnly, DeallocateOnJobCompletion] public NativeArray<byte> data;
    public NativeArray<Pixel32> pixels;
    public PngMetaData metaData;

    public void Execute()
    {
        // NativeArray<Pixel32> pixels = new NativeArray<Pixel32>(metaData.width * metaData.height, Allocator.Persistent);
        ...
        NativeArray<Pixel32> lastLineA = new NativeArray<Pixel32>(metaData.width, Allocator.Temp);
        NativeArray<Pixel32> lastLineB = new NativeArray<Pixel32>(metaData.width, Allocator.Temp);
        NativeArray<Pixel32> currentBuffer = lastLineA;
        NativeArray<Pixel32> lastBuffer = lastLineB;
        ...
        for (int h = 0; h < metaData.height; ++h)
        {
            ...
            for (int x = 0; x < metaData.width; ++x)
            {
                ...
                current = PngParser.GetPixel32(data, startIndex + (x * stride));
...
public struct Pixel32
{
    ...
    public uint4 ToUint4()
    {
        return new uint4(r, g, b, a);
    }

    public static Pixel32 CalculateFloor(Pixel32 left, Pixel32 right)
    {
        uint4 result = (left.ToUint4() + right.ToUint4()) % 256;
        return new Pixel32(result);
    }

    public static Pixel32 CalculateAverage(Pixel32 a, Pixel32 b, Pixel32 c)
    {
        uint4 avg = (b.ToUint4() + c.ToUint4()) / 2;
        return CalculateFloor(a, new Pixel32(avg));
    }
...
public static class PngParser
{
    ...
    private static Texture2D ParseAsRGBA(byte[] rowData, SynchronizationContext unityContext)
    {
        ...
        NativeArray<Pixel32> pdata = new NativeArray<Pixel32>(metaData.width * metaData.height, Allocator.Persistent);

        unityContext.Post(s =>
        {
            Profiler.BeginSample("Create a texture");

            NativeArray<byte> ndata = new NativeArray<byte>(data, Allocator.Persistent);

            PngParseJob job = new PngParseJob
            {
                data = ndata,
                pixels = pdata,
                metaData = metaData,
        ...
        unityContext.Post(s =>
        {
            Pixel32[] result = pdata.ToArrayPooled();
            ...
            try
            {
                IntPtr pointer = handle.AddrOfPinnedObject();
                texture = new Texture2D(metaData.width, metaData.height, TextureFormat.RGBA32, false);
                texture.LoadRawTextureData(pointer, result.Length * 4);
                texture.Apply();
            }
            finally
            {
    ...
    public static Pixel32 GetPixel32(NativeArray<byte> data, int startIndex)
    {
        if (data.Length < startIndex + 4)
        {
            throw new IndexOutOfRangeException($"[{nameof(PngParser)}] Out of range of the data.");
